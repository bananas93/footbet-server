...
router.post('/', checkAuth, upload.single('logo'), async (req, res) => await TeamController.createTeam(req, res)); // Create a new team
router.put('/:id', checkAuth, upload.single('logo'), async (req, res) => await TeamController.updateTeam(req, res)); // Update team
